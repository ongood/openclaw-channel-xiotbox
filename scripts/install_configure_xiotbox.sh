...
Examples:
  bash scripts/install_configure_xiotbox.sh \
    wss://socketd.odoo.games/ws/openclaw dev123 token123 https://api.xiotbox.com 1.0.36 1

  bash scripts/install_configure_xiotbox.sh \
    wss://socketd.odoo.games/ws/openclaw dev123 token123 - \
    https://github.com/ongood/openclaw-channel-xiotbox.git#1.0.36 0

Notes:
  - API_BASE_URL: pass '-' to keep existing config value (or leave empty).
  - TAG_OR_REPO: can be '1.0.36' or full git URL.
MSG
}

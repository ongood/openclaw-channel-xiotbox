...
Examples:
  bash scripts/install_configure_xiotbox.sh \
  wss://socketd.odoo.games/ws/openclaw <DEVICE_ID> <DEVICE_TOKEN> https://api.xiotbox.com 4.0.12 1

  bash scripts/install_configure_xiotbox.sh \
    wss://socketd.odoo.games/ws/openclaw <DEVICE_ID> <DEVICE_TOKEN> - \
  https://github.com/xiotbox/openclaw-channel-xiotbox.git#4.0.12 0

Notes:
  - API_BASE_URL: pass '-' to keep existing config value (or leave empty).
- TAG_OR_REPO: can be '4.0.12' or full git URL.
MSG
}

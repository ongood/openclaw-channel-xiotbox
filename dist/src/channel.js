...
export function resolveSessionCommandAction(commandType) {
...
const conversationBindingRegistry = new Map();
export function rememberConversationBinding(conversationId, binding) {
    const key = String(conversationId || '').trim();
    if (!key)
        return;
    const now = Date.now();
    for (const [k, v] of conversationBindingRegistry.entries()) {
        if (now - v.ts > CONVERSATION_BINDING_TTL_MS)
            conversationBindingRegistry.delete(k);
    }
...
export function lookupConversationBinding(conversationId) {
    const key = String(conversationId || '').trim();
    if (!key)
        return null;
...
export const pendingSessionArchives = new Map();
...
export function settleSessionArchiveAck(data) {
...
function settleSessionArchive(conversationId, ok, error) {
    const key = String(conversationId || '').trim();
...
export function dispatchSessionCommand(ctx, args) {
    ...
    const enterBusinessEntry = () => {
        if (!lifecycle.ackAccepted(cmdId, traceId))
            return false;
...
export const xiotboxPlugin = {
    ...
    outbound: {
        ...
        sendText: async (ctx) => {
            ...
            const resolvedCommandId = (deliveryContext?.commandId || '').trim() ||
                (ctx?.commandId ? String(ctx.commandId) : '') ||
                undefined;
            const resolvedThreadId = (deliveryContext?.threadId || '').trim() ||
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            unregisterDirectSender = registerDirectSender(accountId, ({ text, commandId, threadId, traceId }) => {
                ...
                const resolvedCommandId = String(commandId || '').trim() ||
            ...
            const handleSessionCommand = (args) => {
                dispatchSessionCommand({
                    ...
                    cacheResult: (cmdId, type, payload) => {

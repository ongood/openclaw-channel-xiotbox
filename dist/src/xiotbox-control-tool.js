...
async function jsonRpcCall(baseUrl, path, params, headers, timeoutMs) {
    ...
    return {
        ...
        parameters: {
            ...
            properties: {
                ...
                plan: {
                    type: 'array',
                    description: 'Array of steps: {action, params}. Tool will auto-insert get_tree/get_screen + wait_ui_change around UI actions.',
                    items: {
                        type: 'object',
                        additionalProperties: false,
                        properties: {
        ...
        async execute(_id, rawParams) {
            ...
            const maybeWaitAfter = async (waitTimeoutMs) => {
            ...
            options.logger?.info?.(`[XiotBox] xiotbox_control session=${sessionId} target=${targetDeviceId} steps=${stepsOut.length} ok=${ok} durationMs=${durationMs}`);
            return jsonResult({
                ok,
                session_id: sessionId,

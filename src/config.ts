...
export function normalizeStrList(value: any, fallback: string[]): string[] {
...
export function normalizeThreadId(value?: string | null): string {
  const normalized = String(value || '').trim();
  return normalized || DEFAULT_THREAD_ID;
}

export function normalizeContextEpoch(value?: any): number {
...
export function resolveConversationBinding(
  incoming: any,
  deviceId: string,
): ConversationBindingSnapshot | null {
...
export function normalizePositiveInt(value: any): number | undefined {
  const parsed = Number(value);
...
export function normalizeOptionalBoolean(value: any): boolean | undefined {
...
export function getChannelConfig(cfg: any): any {
  return cfg?.channels?.[CHANNEL_ID] || {};
}

export function buildConfig(channelCfg: any): any {
...
export function resolveAgentId(cfg: any): string {
...
export function readThreadAgentMapValue(map: any, threadId: string): string {
...
export function resolveThreadAgentId(cfg: any, threadId: string): string {
  const channelCfg = getChannelConfig(cfg);
  const mapped =
...
export function resolveAccount(cfg: any, accountId?: string): any {
  const root = getChannelConfig(cfg);

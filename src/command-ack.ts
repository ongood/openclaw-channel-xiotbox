...
export type AckRejectionClass = 'policy' | 'protocol' | 'runtime';

export const ACK_REJECTION_CLASSES: readonly AckRejectionClass[] = [
  'policy',
  'protocol',
  'runtime',
];
...
export type AckRejection = {
  class: AckRejectionClass;
  code: string;
  detail?: string;
};

export type NormalizedRuntimeError = AckRejection;

export type CanonicalAckFrame = {
  type: 'COMMAND_ACK';
  command_id: string;
  accepted: boolean;
...
export type CommandDeliveredFrame = {
...
function trimBoundedDetail(value: unknown): string {
  const text = typeof value === 'string' ? value.trim() : '';
  return text ? text.slice(0, REJECTION_DETAIL_MAX) : '';
}

export function buildAckRejection(
  cls: AckRejectionClass | unknown,
  code: unknown,
  detail?: unknown,
): AckRejection | null {
  if (cls !== 'policy' && cls !== 'protocol' && cls !== 'runtime') return null;
  const trimmedCode = trimId(code);
  if (!trimmedCode) return null;
  const rejection: AckRejection = { class: cls, code: trimmedCode };
  const boundedDetail = trimBoundedDetail(detail);
  if (boundedDetail) rejection.detail = boundedDetail;
  return rejection;
}
...
const RUNTIME_ERROR_CLASS_BY_CODE: Record<string, AckRejectionClass> = {
...
const CAPABILITY_MISMATCH_CODES: ReadonlySet<string> = new Set([
...
export function normalizeRuntimeError(rawCode: unknown): NormalizedRuntimeError {
  ...
  if (cls) return { class: cls, code };
  return { class: 'runtime', code: 'runtime_error', detail: code || 'unknown_error' };
}

export function buildCanonicalAck(
  commandId: unknown,
  opts: {
    accepted: boolean;
    rejection?: AckRejection | null;
    eventId?: unknown;
    traceId?: unknown;
  },
): CanonicalAckFrame | null {
  const id = trimId(commandId);
  if (!id) return null;
  const accepted = opts?.accepted === true;
  const frame: CanonicalAckFrame = {
    type: 'COMMAND_ACK',
    command_id: id,
    accepted,
  };
  ...
  if (eventId) frame.event_id = eventId;
  const traceId = trimId(opts?.traceId);
  if (traceId) frame.trace_id = traceId;
  return frame;
}

export function buildCommandDelivered(commandId: unknown): CommandDeliveredFrame | null {
  const id = trimId(commandId);
  if (!id) return null;
...
export function buildChatUserMessageEventId(commandId: unknown): string | null {
  const id = trimId(commandId);
  if (!id) return null;
  return `${id}:${CHAT_USER_MESSAGE_OCCURRENCE_ID}`;
}

export type CommandSendFn = (type: string, payload: any) => void;

type CommandLifecycleEmitterOptions = {
  now?: () => number;
  ttlMs?: number;
  // Capacity cap for EVICTABLE entries only (rejection/DELIVERED evidence and
  // TERMINAL ownership). ACTIVE execution ownership is never a victim.
  maxEntries?: number;
};
...
type RejectedAckEntry = { frame: CanonicalAckFrame; ts: number };
type DeliveredEntry = { frame: CommandDeliveredFrame; ts: number };

// Execution ownership for a claimed command. While `terminal` is false the
// entry is ABSOLUTELY protected: it is never dropped by TTL or capacity
// pressure (XIOT-BUG-0050b R1 round 3). Only markTerminal() flips it into the
// evictable pool — after the caller has cached the terminal result — and from
// then on it ages out exactly like the command-result cache.
type OwnershipEntry = {
  frame: CanonicalAckFrame;
  claimedAt: number;
  terminal: boolean;
  terminalAt?: number;
};
...
export class CommandLifecycleEmitter {
  private readonly send: CommandSendFn;
  private readonly now: () => number;
  private readonly ttlMs: number;
  private readonly maxEntries: number;
  private readonly rejectedAcks = new Map<string, RejectedAckEntry>();
  private readonly ownership = new Map<string, OwnershipEntry>();
  private readonly deliveredFrames = new Map<string, DeliveredEntry>();

  constructor(send: CommandSendFn, opts?: CommandLifecycleEmitterOptions) {
  ...
  private prune(): void {
    ...
    for (const [id, entry] of this.rejectedAcks) {
      if (entry.ts < cutoff) this.rejectedAcks.delete(id);
    }
    for (const [id, entry] of this.ownership) {
      ...
      if (entry.terminal && (entry.terminalAt ?? 0) < cutoff) this.ownership.delete(id);
    }
    for (const [id, entry] of this.deliveredFrames) {
      if (entry.ts < cutoff) this.deliveredFrames.delete(id);
    }
  ...
  private evictEvictableOverflow<T extends { ts: number }>(map: Map<string, T>, tsOf: (entry: T) => number): void {
  ...
  // work).
  private evictOwnershipOverflow(): void {
  ...
  ackAccepted(commandId: unknown, traceId?: unknown): boolean {
    const id = trimId(commandId);
    if (!id) return false;
    ...
    if (existing) {
      ...
      if (delivered) this.send(delivered.frame.type, delivered.frame);
      return false;
    }
  ...
  isAccepted(commandId: unknown): boolean {
  ...
  markTerminal(commandId: unknown): boolean {
    const id = trimId(commandId);
    if (!id) return false;
  ...
  // instead.
  replayLifecycleEvidence(commandId: unknown): boolean {
    const id = trimId(commandId);
    if (!id) return false;
  ...
  ackRejected(
    commandId: unknown,
    rejection: AckRejection | null | undefined,
    traceId?: unknown,
  ): CanonicalAckFrame | null {
    const id = trimId(commandId);
    if (!id) return null;
  ...
  replayRejectedAck(commandId: unknown): boolean {
    const id = trimId(commandId);
    if (!id) return false;
  ...
  hasRejectedAck(commandId: unknown): boolean {
  ...
  markDelivered(commandId: unknown): boolean {
    const id = trimId(commandId);
    if (!id) return false;
    if (this.rejectedAcks.has(id)) return false;
    if (this.deliveredFrames.has(id)) return false;
  ...
  hasDelivered(commandId: unknown): boolean {

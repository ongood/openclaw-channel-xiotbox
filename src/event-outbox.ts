...
type DurableEventOutboxOptions = {
  filePath: string;
...
export function resolveEventOutboxPath(deviceId: string): string {
...
export class DurableEventOutbox {
  ...
  private readonly send: (payload: Record<string, any>) => void;
  ...
  private readonly entries = new Map<string, PendingEvent>();
  private timer: NodeJS.Timeout | null = null;

  constructor(options: DurableEventOutboxOptions) {
  ...
  enqueue(payload: Record<string, any>): void {
    ...
    if (!eventId) throw new Error('event_id required for durable outbox');
    ...
    if (isNew) this.flushEvent(eventId, true);
  }

  acknowledge(ack: EventAck): boolean {
    const eventId = String(ack?.event_id || '').trim();
    const status = String(ack?.status || '').trim().toLowerCase();
    if (!eventId || !['accepted', 'rejected'].includes(status)) return false;
  ...
  private flushEvent(eventId: string, force: boolean): void {
    const entry = this.entries.get(eventId);
    if (!entry) return;
    const now = this.now();
    if (!force && entry.nextAttemptAt > now) return;
    try {
      this.send(entry.payload);
      entry.attempts += 1;
      const delay = Math.min(
        this.retryBaseMs * (2 ** Math.min(entry.attempts - 1, 16)),
        this.retryMaxMs,
      );
  ...
  private prune(now: number): void {
    for (const [eventId, entry] of this.entries) {
      if (now - entry.createdAt > this.ttlMs) this.entries.delete(eventId);
    }
  ...
  private load(): void {
    try {
      ...
      const parsed = JSON.parse(fs.readFileSync(this.filePath, 'utf8')) as EventOutboxFile;
      if (parsed?.version !== 1 || !Array.isArray(parsed.entries)) return;
      const now = this.now();
      for (const entry of parsed.entries) {
        const eventId = String(entry?.payload?.event_id || '').trim();
        if (!eventId || !Number.isFinite(entry?.createdAt)) continue;
        this.entries.set(eventId, {
  ...
  private persist(): void {
    ...
    const data: EventOutboxFile = { version: 1, entries: [...this.entries.values()] };
